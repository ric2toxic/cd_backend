...
list(,,$consumer_count) = $channel->queue_declare('order_backup_queue', false, true, false, false);
...
$callback = function($msg){
    ...
    while ($success == false) {
        try{
            ...
            $selector = array();
            $order_data = OrderInfo::getOrderInfo($this->db, $order_id, '', $selector);
            foreach ($order_data['suborder'] as $suborder_id => $suborder ) {

                $order_info = array();
                $order_info['order'] = $order_data['order'];
                $order_info['suborder'] = array();
                $order_info['suborder'][$suborder_id] = $suborder;
                $order_info = json_encode($order_info);

                $sql = "INSERT INTO " . DB_PREFIX . "order_backup
                        SET order_id = '".(int)$order_id."',
                            suborder_id = '".$this->db->escape($suborder_id)."',
                            original_order_data = '".$this->db->escape($order_info)."',
                            date_added = NOW() ";
