...
class ActionGoodsTakenOnWsbAndRelisted extends ReturnActionBase 
{	
	...
	public function checkPostActionVisibility($return_id, $data, $return_action_id, $check_for){
    	...
    	if(!empty($return_id)){
    		...
    		if(
    			...
    			return $is_visible; //This ReturnAction will not show as PostAction
    		}
	...
	*/
	public function sendEmail($data = array()) {
		if(!empty($data)) {
			...
			$mailer->emailSetting(
								array(
									'subject' 	=> 'Return/Replacement request for Order No '.$data['order_no'].' - Status Updated: Goods Taken On WSB Books and Relisted',
									array('returns', 'accounts', 'vikas','sellers', 'prabhav'),
									'template'	=> '',
	 ...
	 * @author: Nishu, March 2018
	*/
	private function email_template($mailer, $data)
	{
		$_html = '<div style="font-family:Arial,Helvetica,sans-serif;max-width:680px">';
		$_html .= $mailer->getHeaderText('All');
		$_html .= '<p>';
		$_html .= 'Goods Taken On WSB Books and Relisted for Order No '.$data['order_no'].'.<br><br>';
		$_html .= 'Following products are created with disabled status, need to re-set options (Like: Quantity, Piece in Set and product options) manually and enable to show on website.';	
		$_html .= '</p>';
		$_html .= '<h4>Products:</h4>';
