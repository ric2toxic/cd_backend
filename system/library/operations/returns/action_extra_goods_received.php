...
class ActionExtraGoodsReceived extends ReturnActionBase 
{	
	protected $action_name 	  = 'Extra goods received';
	...
	*/
	public function customer_email_template($mailer, $data)
	{
		...
		$_html .= '<p>';
		$_html .= 'There is an update against your Return/Replacement request for the Order No '.$data['order_no'].'. We have received products as per your request and we also received extra goods. ';
		$_html .= 'We shall send extra goods back to you soon.';
		$_html .= '</p>';
	...
	*/
	public function customer_sms_template($data)
	{
		...
		$_message =  sprintf(
							RETURN_CUSTOMER_SMS_TEMPLATE[RETURN_ACTION_IDS['Extra_Goods_Received']],
							$order_no,
							$url,
							RETURN_HELPLINE
						);
	...
	*/
	...
	/**
	 * @info: Public Method send customer push notification
	 * @param: $data Array
	 * @return: String SMS template
	 * @author: MSA, March 2018
	*/
	public function customer_notification($data)
	{
		...
		$message = 'We have received extra products as per your return request for Order No '.$order_no.'.';
