...
class ActionGoodsReceived extends ReturnActionBase 
{	
	protected $action_name 	  = 'Goods received';
		...
		$_html = '<div style="font-family:Arial,Helvetica,sans-serif;max-width:680px">';
			...
			$_html .= $mailer->getHeaderText( ucfirst($data['customer']['name']) );
				...
				$_html .= '<p>';
					...
					$_html .= 'We have received products as per your request. After analyzing quality of returned product(s), We shall take appropriate action. ';
				$_html .= '</p>';
	...
	*/
	public function customer_sms_template($data)
	{
		...
		$_message =  sprintf(
							RETURN_CUSTOMER_SMS_TEMPLATE[RETURN_ACTION_IDS['Goods_Received']],
							$order_no,
							$url,
							RETURN_HELPLINE
						);
	...
	*/
	public function sendPushNotification($data = array())
	{
		...
		$message = 'We have received products as per your return request for Order No '.$order_no.'. ';
