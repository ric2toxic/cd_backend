...
class ActionReturnRequestRejected extends ReturnActionBase 
{	
	...
	*/
	...
	{	
		...
		$all_return_ids = $this->getReturnLastStatus($data['returns']);
	...
	/**
	 * @info: Public Method to create customer email contents
	 * @param: $data Array
	 * @return: String customer email template
	 * @author: MSA, March 2018
	*/
	public function customer_email_template($mailer, $data)
	{
		$_html = '<div style="font-family:Arial,Helvetica,sans-serif;max-width:680px">';
			$_html .= $mailer->getHeaderText( ucfirst($data['customer']['name']) );
				$_html .= '<p>';
					$_html .= 'There is an update against your Return request for the Order No '.$data['order_no'].'. ';
	...
	/**
	 * @info: Public Method to create seller email contents
	 * @param: $data Array
	 * @return: String seller email template
	 * @author: MSA, March 2018
	*/
	...
	*/
	private function customer_sms_template($data)
	{
		...
		$_message =  sprintf(
							RETURN_CUSTOMER_SMS_TEMPLATE[RETURN_ACTION_IDS['Return_Request_Rejected']],
							$number_of_pieces,
							$order_no,
							$url,
							RETURN_HELPLINE
	...
	*/
	...
	} // end of seller notification method
