...
class ActionTransferToWSBBooksWithoutRelisting extends ReturnActionBase 
{	
	...
	*/
	public function sendEmail($data = array()) {
		if(!empty($data)) {
			...
			$mailer->emailSetting(
								array(
									'subject' 	=> 'Return/Replacement request for Order No '.$data['order_no'].' - Status Updated: Goods Taken On WSB Books Without Relisted',
	...
	*/
	private function email_template($mailer, $data)
	{
		...
		$_html .= '<p>';
		$_html .= 'Goods Taken On WSB Books Without Relisted for Order No '.$data['order_no'].'.<br><br>';
		$_html .= '</p>';
