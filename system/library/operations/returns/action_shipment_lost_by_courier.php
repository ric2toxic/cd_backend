...
class ActionShipmentLostbyCourier extends ReturnActionBase 
{	
	protected $action_name 	  = 'Request Cancelled';
	...
	*/
	public function sendEmail($data = array()) 
	{
		...
		if(!empty($seller_data))
		{
			foreach ($seller_data as $key => $value) 
			{
				...
				$mailer->emailSetting(
									array(
											'subject' 	  => 'WholesaleBox :: Return/Replacement request for Order No '.$value['order_no'].' - Status Updated: '.$this->action_name,
	...
	/**
	 * @info: Public Method to set email template for seller
	...
	private function seller_email_template($mailer, $data)
	{
		$_html = '<div style="font-family:Arial,Helvetica,sans-serif;max-width:680px">';
			$_html .= $mailer->getHeaderText($data['seller']['name']);
				$_html .= '<p>';
					$_html .= 'We have cancelled the return/replacement request, for the Order No '.$data['order_no'].'. ';
					$_html .= 'In previous emails, tentative details of the return request received from the customer, were already informed to you. 
								Please find below details of the cancelled return request items.';
				$_html .= '</p>';
					...
					$master_return_id = array_unique(array_column($seller_returns['returns'], 'master_return_id'));
						$message = array(
				           'custom_notification' => array
										           (
										               'sound' 					=> 'default',
										               'title'     				=> 'Return request has been cancelled for Order No ' . $order_no,
										               'body'    				=> 'Return request has been cancelled for Order No ' . $order_no,
