...
class ActionReplacementRequestAccepted extends ReturnActionBase 
{	
	protected $action_name 	  = 'Accepted';
	protected $support_number = RETURN_HELPLINE;
		...
		$_html = '<div style="font-family:Arial,Helvetica,sans-serif;max-width:680px">';
			$_html .= $mailer->getHeaderText( ucfirst($data['customer']['name']) );
				$_html .= '<p>';
					...
					$_html .= 'Your replacement request has been '.$this->action_name.' ';
					$_html .= 'for following products. ';
				$_html .= '</p>';
			$_html .= $mailer->getProductList($data);
			
			$_html .= '<p>Please wait for 48 hours to get next status about your replacement. Kindly pack the returned products and keep ready, if you have opted for Wholesalebox pick up and if you have opted for self shipment then we shall update you the shipping address of wholesalebox.</p>';
	...
	private function seller_email_template($mailer, $data) {
		$_html = '<div style="font-family:Arial,Helvetica,sans-serif;max-width:680px">';
			$_html .= $mailer->getHeaderText( ucfirst($data['seller']['name']) );
				$_html .= '<p>';
					$_html .= 'There is a replacement request for the Order No '.$data['order_no'].'. ';
					$_html .= 'In previous emails, tentative details of the replacement request received from the customer, ';
					$_html .= 'were already informed to you. Please find below, the current details of the items to be replaced, as per the customer’s request.';
				$_html .= '</p>';
			$_html .= $mailer->getSellerProductList($data);
			$_html .= '<p>Goods shall be given to you for replacement, in few working days. Kindly keep replacement ready.</p>';
	...
	*/
	private function customer_sms_template($data) {
		...
		$_message =  sprintf(
							RETURN_CUSTOMER_SMS_TEMPLATE[RETURN_ACTION_IDS['Replacement_Request_Accepted']],
							$order_no,
							$number_of_pieces,
							$url,
							RETURN_HELPLINE
	...
	*/
	public function customer_notification($data)
	{
		...
		$message = 'Your replacement request has been '.$this->action_name.' for Order No '.$order_no.'. ';
