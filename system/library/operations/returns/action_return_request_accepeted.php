...
class ActionReturnRequestAccepeted extends ReturnActionBase 
{	
	protected $action_name 	  = 'Accepted';
	...
	{	
		...
		if(!empty($seller_data))
		{
			...
			foreach ($seller_data as $key => $value) 
			{
				...
				$mailer->emailSetting(
									array(
											'subject' 	  => 'WholesaleBox :: Return request for Order No '.$value['order_no'].' - Status Updated: '.$this->action_name,
		...
		$_html = '<div style="font-family:Arial,Helvetica,sans-serif;max-width:680px">';
			$_html .= $mailer->getHeaderText( ucfirst($data['customer']['name']) );
				$_html .= '<p>';
					...
					$_html .= 'Your return request has been '.$this->action_name.' ';
					$_html .= 'for following products. ';
				$_html .= '</p>';
			$_html .= $mailer->getProductList($data);
			$_html .= '<p>Please wait for 48 hours to get next status about your return. Kindly pack the returned products and keep ready, if you have opted for Wholesalebox pick up and if you have opted for self shipment then we shall update you the shipping address of wholesalebox.</p>';
	...
	private function seller_email_template($mailer, $data)
	{
		$_html = '<div style="font-family:Arial,Helvetica,sans-serif;max-width:680px">';
			$_html .= $mailer->getHeaderText( ucfirst($data['seller']['name']) );
				$_html .= '<p>';
					$_html .= 'There is a return request for the Order No '.$data['order_no'].'. ';
					$_html .= 'In previous emails, tentative details of the return request received from the customer, 
								were already informed to you. Please find below, 
								the current details of the return request accepted items, 
								as per the customer’s request.';
				$_html .= '</p>';
			$_html .= $mailer->getSellerProductList($data);
			$_html .= '<p>Our internal team shall inspect the goods, 
						before returning back to you. 
						If returns from the customer are found satisfactory, 
						our pickup person shall hand over the goods back to you with physical copy of debit note, 
						in few working days.</p>';
	...
	*/
	private function customer_sms_template($data)
	{
		...
		$_message =  sprintf(
							RETURN_CUSTOMER_SMS_TEMPLATE[RETURN_ACTION_IDS['Return_Request_Accepted']],
							$order_no,
							$number_of_pieces,
							$url,
							RETURN_HELPLINE
					...
					$master_return_id = array_unique(array_column($seller_returns['returns'], 'master_return_id'));
						$message = array(
				           'custom_notification' => array
										           (
										               'sound' 					=> 'default',
										               'title'     				=> 'Return request has been '.$this->action_name.' for Order No ' . $order_no,
										               'body'    				=> 'Return request has been '.$this->action_name.' for Order No ' . $order_no,
