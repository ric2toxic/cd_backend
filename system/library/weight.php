...
class Weight {
	...
	public function __construct($registry) {
		...
		$weight_cache = $this->cache->get('weight_class_' . $this->config->get('config_language_id'));

		if (empty($weight_cache)) {
			$weight_class_query = $this->db->query("SELECT
          	                                      wc.weight_class_id,
          	                                      wc.value,
          	                                      wcd.title,
          	                                      wcd.unit
			...
			$weight_cache = $weight_class_query->rows;
			$this->cache->set('weight_class_' . $this->config->get('config_language_id'), $weight_cache);

		}

		foreach ($weight_cache as $result) {
