...
class Length {
	...
	public function __construct($registry) {
		...
		$length_cache = $this->cache->get('length_class_'.$this->config->get('config_language_id'));

        if(empty($length_cache))
        {
		  $length_class_query = $this->db->query("SELECT 
		                                           ...
		                                           mcd.unit 
		  	                                      FROM " . DB_PREFIX . "length_class mc
		  	                                       LEFT JOIN " . DB_PREFIX . "length_class_description  
		  	                                       mcd ON (mc.length_class_id = mcd.length_class_id) 
		  	                                       WHERE mcd.language_id = '" . (int)$this->config->get('config_language_id') . "'");

		  $length_cache = $length_class_query->rows;
          $this->cache->set('length_class_'.$this->config->get('config_language_id'), $length_cache);
		}  


		foreach ($length_cache as $result) 
		{
