...
list(,,$consumer_count) = $channel->queue_declare('order_solr_sync_queue', false, true, false, false);
//echo $consumer_count;

// exit if already max_consumers listening onto this queue
if ($consumer_count >= MAX_CONSUMERS)
	exit('Max Consumer Limit Reached');

//echo ' [*] Waiting for emails to send. To exit press CTRL+C', "\n";

$callback = function($msg){
	echo " [x] Received ", $msg->body, "\n";
	$changes = unserialize(base64_decode($msg->body));
	try {
        $object = new stdClass();
        $object->solr_config = array(
                            'endpoint' => array(
                                'localhost' => array(
                                        'host' => 'localhost',
                                        'port' => 8983,
        ...
        $solr_order = new SolrOrder($object);
        if(!empty($changes)){
            if(!empty($changes['change_id'])){
                $change_id = $changes['change_id'];
                unset($changes['change_id']);
            }

            if($solr_order->atomicUpdateToSolr($changes)){
		...
		error_log('\n' . $e->getMessage() . '\n Serialized Email Object: ' . base64_decode($msg->body),
		          3,
		          __DIR__ . '/order_solr_sync_queue.log');
	}
	//echo " [x] Done", "\n";
...
$channel->basic_consume('order_solr_sync_queue', '', false, false, false, false, $callback);
