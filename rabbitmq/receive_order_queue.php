...
require_once(DIR_SYSTEM . 'library/updateorder.php');
...
list(,,$consumer_count) = $channel->queue_declare('order_updation_queue', false, true, false, false);
...
$callback = function($msg){
	$upd_order = unserialize(base64_decode($msg->body));
	
	$success = false;
	while ($success == false) {
        $upd_order->loadDb();
		$success = $upd_order->updateOrderData(false); // false means dont send to queue
	}
...
$channel->basic_consume('order_updation_queue', '', false, false, false, false, $callback);
