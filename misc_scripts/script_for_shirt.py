...
#######################################################################################################################
#######################################################################################################################
manufacturer_code =	"DEA_JP"																			###############
																										###############	
whether_img_folder  = 1		# 1 tells whether the images are in sku folders								###############
...
# b = ['Fabric Type', 'Fabric Type','Set Type', 'Style','Sleeve','Work Type', 'Length', 'Neck Type']
b = ['Fabric Type','Set Type','Style','Sleeve','Work Type','Length','Color']
...
####---------category_id- 63 : Shirts																			###############
####---------category_id- 67 : Leggings																			###############
####---------category_id- 66																			###############
																										###############	
TAX_CLASS = 9 		# Tax class id																		###############
SELL_TAX = 5		# Seller Tax																		###############
COMM = 7			# Commission 																		###############																					
CAT_ID = 63			# Category id	
																										###############
...
### ------The local_image_root is for image directory in local machine ----###							###############	
																										###############
server_image_root = "catalog/"+manufacturer_code+"/shirt/"											###############
local_image_root = "/Users/qurius/Documents/" +manufacturer_code+"/shirt/"							###############
																										###############
																										###############
#######################################################################################################################
#######################################################################################################################
	...
	for row in reader:
		...
		if rownum == 0:
			...
			for key,value in zip(header, row[0:first_index]):
				product_dict[key] = " ".join(value.split())
...
######################################################################
cnx = MySQLdb.connect(user= 'root',passwd = 'p341987g',host = 'localhost',db = 'deeksha', conv = my_conv)
...
###### -FOR PRODUCT TABLE ONLY----------------############		
		...
		#######################################################################################
			...
			if '.DS_Store' in filenames_sorted:
				filenames_sorted.remove('.DS_Store')
		...
		############################################################################################
			...
			if '.DS_Store' in related_filenames_sorted:
	 			related_filenames_sorted.remove('.DS_Store')
			...
			for three, four in enumerate(filter_common_list):
				if filter_common_list[three] in two.values():
					for five,six in enumerate(rows_cursor_filter_description_table):
						if (" ".join(product_list_variable[i][three].split()) in set(six.values()))& ((two['filter_group_id'] in six.values())):
						...
						if (product_list_variable[i][three] not in set(six.values()))& ((two['filter_group_id'] in six.values())):
