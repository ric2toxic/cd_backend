...
require_once __DIR__ . '/system/library/db/db.php';
require_once __DIR__ . '/system/library/updatelead.php';
...
list(,,$consumer_count) = $channel->queue_declare('lead_updation_queue', false, true, false, false);
...
$callback = function($msg){
	$upd_lead = unserialize(base64_decode($msg->body));
	
	$success = false;
	while ($success == false) {
        $upd_lead->loadDb();
		$success = $upd_lead->updateLeadData(false); // false means dont send to queue
	}
        if($success == "success"){
        }else{
            $success = $upd_lead->insertLeadUpdation();
        }
...
$channel->basic_consume('lead_updation_queue', '', false, false, false, false, $callback);
