...
class ModelCatalogReview extends Model {
	...
	public function getReviews($data = array()) {
		$sql = "SELECT r.review_id, p.model, r.author, r.rating, r.status, p.date_added FROM " . DB_PREFIX . "review r INNER JOIN " . DB_PREFIX . "product p ON (r.product_id = p.product_id) WHERE r.store_id IN (".WSB_STORES_ID.")";

		if (!empty($data['filter_product'])) {
			$sql .= " AND p.model LIKE '%" . $this->db->escape($data['filter_product']) . "%'";
		}
	...
	public function getTotalReviews($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r INNER JOIN " . DB_PREFIX . "product p ON (r.product_id = p.product_id) 
                WHERE r.store_id IN (".WSB_STORES_ID.")";
