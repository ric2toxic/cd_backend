...
class ModelLocalisationTaxClass extends Model {
	...
	public function editTaxClass($tax_class_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "tax_class SET title = '" . $this->db->escape($data['title']) . "', description = '" . $this->db->escape($data['description']) . "', status = '" . (int)($data['status']) ."', date_modified = NOW() WHERE tax_class_id = '" . (int)$tax_class_id . "'");
	...
	public function getTaxClasses($data = array()) {
		if ($data) {
			...
			if (!$tax_class_data) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tax_class WHERE status = 1");
