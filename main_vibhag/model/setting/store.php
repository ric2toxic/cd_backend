...
class ModelSettingStore extends Model {
	...
	public function getExtensions($store_id=0, $type='payment'){
		$where = '';
		if($store_id > 0){
			$where = ' AND store_id = '. $store_id;
		}

		 $q = "SELECT e.extension_id, e.code, e2s.store_id FROM ".DB_PREFIX."extension e ".
				" LEFT JOIN ".DB_PREFIX."wsb_extension_to_store e2s".
				" ON e2s.extension_id = e.extension_id".
	...
	public function getSelectedExtensions($store_id=0, $type='payment'){
		if($store_id > 0){
			$where = ' AND store_id = '. $store_id;
		}

		$q = "SELECT e.extension_id  FROM ".DB_PREFIX."extension e ".
				" LEFT JOIN ".DB_PREFIX."wsb_extension_to_store e2s".
				" ON e2s.extension_id = e.extension_id".
				" WHERE e.type='payment'".
				$where.
	 ...
	 * This Function is a copy of a front end model function to prevent call to a front end model.
	 */

	public function getInternationalSwitch() {
		return "khufiya_vibhag";
	}
}