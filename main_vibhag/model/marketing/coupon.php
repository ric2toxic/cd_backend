...
class ModelMarketingCoupon extends Model {
	public function addCoupon($data) {
        ...
        if(isset($data['coupon_customer'])){
            foreach ($data['coupon_customer'] as $customer_id) {
                $sql = "INSERT INTO " . DB_PREFIX . "coupon_customer SET coupon_id = '" . (int)$coupon_id . "', customer_id = '" . (int)$customer_id . "'";
                exit($sql);
				$this->db->query();
			}
        }
	...
	public function getCoupon($coupon_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "coupon WHERE coupon_id = '" . (int)$coupon_id . "' AND store_id IN (".WSB_STORES_ID .")");

		return $query->row;
	}

	public function getCouponByCode($code) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "coupon WHERE code = '" . $this->db->escape($code) . "' AND store_id IN (".WSB_STORES_ID .")");

		return $query->row;
	}

	public function getCoupons($data = array()) {
		$sql = "SELECT coupon_id, name, code, discount, date_start, date_end, status FROM " . DB_PREFIX . "coupon WHERE store_id IN (".WSB_STORES_ID .")";
    ...
    public function getCouponCustomers($coupon_id) {
		...
		foreach ($query->rows as $result) {
			$coupon_category_data[] = $result['customer_id'];
		}
	...
	public function getTotalCoupons() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "coupon WHERE store_id IN (".WSB_STORES_ID .")");
