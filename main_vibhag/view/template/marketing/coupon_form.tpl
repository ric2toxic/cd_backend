              ...
              <div class="form-group">
                <label class="col-sm-2 control-label" for="input-customer">
                    <span data-toggle="tooltip"
                          title="Choose specific customers the coupon will apply to. Select no customers to apply coupon to entire cart.">
                        Customers</span>
                    </label>
                <div class="col-sm-10">
                  <input type="text"
                  ...
                  <div id="coupon-customer" class="well well-sm" style="height: 150px; overflow: auto;">
                    <?php foreach ($coupon_customers as $coupon_customer) { ?>
                    <div id="coupon-customer<?php echo $coupon_customer['customer_id']; ?>">
                        <i class="fa fa-minus-circle"></i> <?php echo $coupon_customer['name']; ?>
...
$('input[name=\'customer\']').autocomplete({
	'source': function(request, response) {
        if(request.length >= 3){
            $.ajax({
    			url: 'index.php?route=marketing/coupon/autocompleteCustomer&token=<?php echo $token; ?>&filter_all=' +  encodeURIComponent(request),
    			dataType: 'json',
    			success: function(json) {
    				response($.map(json, function(item) {
    					return {
