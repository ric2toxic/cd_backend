...
class ControllerOpenbayAmazonListing extends Controller {
	public function create() {
	    ...
	    $this->load->autoLoadLanguage('openbay/amazon_listing', $data);
		...
		if ($this->request->post) {
			...
			if ($result['status'] === 1) {
				$this->session->data['success'] = $data['text_product_sent'];
				$this->response->redirect($this->url->link('extension/openbay/items', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			} else {
				$this->session->data['error'] = sprintf($data['text_product_not_sent'], $result['message']);
		...
		$data['conditions'] = array(
			'New' => $data['text_new'],
			'UsedLikeNew' => $data['text_used_like_new'],
			'UsedVeryGood' => $data['text_used_very_good'],
			'UsedGood' => $data['text_used_good'],
			'UsedAcceptable' => $data['text_used_acceptable'],
			'CollectibleLikeNew' => $data['text_collectible_like_new'],
			'CollectibleVeryGood' => $data['text_collectible_very_good'],
			'CollectibleGood' => $data['text_collectible_good'],
			'CollectibleAcceptable' => $data['text_collectible_acceptable'],
			'Refurbished' => $data['text_refurbished'],
		);

		$data['marketplaces'] = array(
			'uk' => $data['text_united_kingdom'],
			'de' => $data['text_germany'],
			'fr' => $data['text_france'],
			'it' => $data['text_italy'],
			'es' => $data['text_spain'],
		);
	...
	public function edit() {
		...
		$this->document->setTitle($data['text_edit_heading']);
		$this->document->addScript('view/javascript/openbay/js/faq.js');
