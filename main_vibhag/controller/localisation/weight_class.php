...
class ControllerLocalisationWeightClass extends Controller {
	...
	protected function getList() {
	    ...
	    $this->load->autoLoadLanguage('localisation/weight_class', $data);
		...
		foreach ($results as $result) {
			$data['weight_classes'][] = array(
				...
				'title'           => $result['title'] . (($result['weight_class_id'] == $this->config->get('config_weight_class_id')) ? $data['text_default'] : null),
				'unit'            => $result['unit'],
				'value'           => $result['value'],
		...
		$data['results'] = sprintf($data['text_pagination'], ($weight_class_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($weight_class_total - $this->config->get('config_limit_admin'))) ? $weight_class_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $weight_class_total, ceil($weight_class_total / $this->config->get('config_limit_admin')));
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['weight_class_id']) ? $data['text_add'] : $data['text_edit'];
