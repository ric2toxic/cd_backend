...
class ControllerLocalisationZone extends Controller {
	...
	protected function getList() {
	    ...
	    $this->load->autoLoadLanguage('localisation/zone', $data);
		...
		$data['action'] = $this->url->link('localisation/zone', 'token=' . $this->session->data['token'], 'SSL');
		$data['search'] = '';
		...
		foreach ($results as $result) {
			$data['zones'][] = array(
				...
				'name'    => $result['name'] . (($result['zone_id'] == $this->config->get('config_zone_id')) ? $data['text_default'] : null),
				'code'    => $result['code'],
		...
		$data['results'] = sprintf($data['text_pagination'], ($zone_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($zone_total - $this->config->get('config_limit_admin'))) ? $zone_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $zone_total, ceil($zone_total / $this->config->get('config_limit_admin')));
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['zone_id']) ? $data['text_add'] : $data['text_edit'];
