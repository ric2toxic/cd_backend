...
class ControllerLocalisationTaxClass extends Controller {
	...
	protected function getList() {
	    ...
	    $this->load->autoLoadLanguage('localisation/tax_class', $data);
		...
		$data['sort_status'] = $this->url->link('localisation/tax_class', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		...
		$data['results'] = sprintf($data['text_pagination'], ($tax_class_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($tax_class_total - $this->config->get('config_limit_admin'))) ? $tax_class_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $tax_class_total, ceil($tax_class_total / $this->config->get('config_limit_admin')));
	...
	protected function getForm() {
		...
		if (isset($this->request->post['status'])) {
			...
			$data['status'] = $tax_class_info['status'];
		} else {
			$data['status'] = '';
		}
