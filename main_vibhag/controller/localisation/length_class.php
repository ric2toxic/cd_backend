...
class ControllerLocalisationLengthClass extends Controller {
	...
	protected function getList() {
	    ...
	    $this->load->autoLoadLanguage('localisation/length_class', $data);
		...
		foreach ($results as $result) {
			$data['length_classes'][] = array(
				...
				'title'           => $result['title'] . (($result['length_class_id'] == $this->config->get('config_length_class_id')) ? $data['text_default'] : null),
				'unit'            => $result['unit'],
				'value'           => $result['value'],
		...
		$data['results'] = sprintf($data['text_pagination'], ($length_class_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($length_class_total - $this->config->get('config_limit_admin'))) ? $length_class_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $length_class_total, ceil($length_class_total / $this->config->get('config_limit_admin')));
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['length_class_id']) ? $data['text_add'] : $data['text_edit'];
