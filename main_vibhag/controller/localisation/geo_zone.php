...
class ControllerLocalisationGeoZone extends Controller {
	...
	protected function getList() {
	    ...
	    $this->load->autoLoadLanguage('localisation/geo_zone', $data);
		...
		$data['action'] = $this->url->link('localisation/geo_zone', 'token=' . $this->session->data['token'], 'SSL');
		if (isset($this->request->get['search']) && $this->request->get['search'] != '') {
			$data['search'] = $this->request->get['search'];
			$filter_data['search'] = $this->request->get['search'];
			$results = $this->model_localisation_geo_zone->getGeoZonesNew($filter_data);
		} else {
			$data['search'] = '';
		...
		$data['results'] = sprintf($data['text_pagination'], ($geo_zone_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($geo_zone_total - $this->config->get('config_limit_admin'))) ? $geo_zone_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $geo_zone_total, ceil($geo_zone_total / $this->config->get('config_limit_admin')));
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['geo_zone_id']) ? $data['text_add'] : $data['text_edit'];
