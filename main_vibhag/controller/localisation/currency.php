...
class ControllerLocalisationCurrency extends Controller {
	...
	protected function getList() {
	    ...
	    $this->load->autoLoadLanguage('localisation/currency',$data);
		...
		$data['results'] = sprintf($data['text_pagination'], ($currency_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($currency_total - $this->config->get('config_limit_admin'))) ? $currency_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $currency_total, ceil($currency_total / $this->config->get('config_limit_admin')));
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['currency_id']) ? $data['text_add'] : $data['text_edit'];
		...
		if (isset($this->request->post['conversion_rate'])) {
			...
			$data['conversion_rate'] = $currency_info['conversion_rate'];
		} else {
		...
		if (isset($this->request->post['country_code'])) {
			...
			$data['country_code'] = $currency_info['country_code'];
		} else {
			$data['country_code'] = '';
		}
	...
	protected function validateForm() {
		...
		if (utf8_strlen($this->request->post['country_code']) != 2) {
