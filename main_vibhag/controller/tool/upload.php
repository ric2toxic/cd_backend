...
class ControllerToolUpload extends Controller {
	...
	protected function getList() {
	    ...
	    $this->load->autoLoadLanguage('tool/upload', $data);
		if (isset($this->request->get['filter_name'])) {
		...
		$data['results'] = sprintf($data['text_pagination'], ($upload_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($upload_total - $this->config->get('config_limit_admin'))) ? $upload_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $upload_total, ceil($upload_total / $this->config->get('config_limit_admin')));
