...
class ControllerMarketingAffiliate extends Controller {
	...
	protected function getList() {
	    ...
	    $this->load->autoLoadLanguage('marketing/affiliate', $data);
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['affiliate_id']) ? $data['text_add'] : $data['text_edit'];
		...
		if (isset($this->error['uses_customer'])) {
			$data['error_uses_customer'] = $this->error['uses_customer'];
		} else {
			$data['error_uses_customer'] = '';
		}
		...
		if (isset($this->request->post['discount_amount'])) {
			...
			$data['discount_amount'] = $affiliate_info['discount_amount'];
		} else {
		...
		if (isset($this->request->post['uses_customer'])) {
			...
			$data['uses_customer'] = $affiliate_info['uses_customer'];
		} else {
			$data['uses_customer'] = '';
		}
	...
	public function transaction() {
		...
		$data['results'] = sprintf($data['text_pagination'], ($transaction_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($transaction_total - 10)) ? $transaction_total : ((($page - 1) * 10) + 10), $transaction_total, ceil($transaction_total / 10));
