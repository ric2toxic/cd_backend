...
class ControllerCatalogDownload extends Controller {
	...
	protected function getList() {
        ...
        $this->load->autoLoadLanguage('catalog/download', $data);
		if (isset($this->request->get['sort'])) {
		...
		$data['results'] = sprintf($data['text_pagination'], ($download_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($download_total - $this->config->get('config_limit_admin'))) ? $download_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $download_total, ceil($download_total / $this->config->get('config_limit_admin')));
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['download_id']) ? $data['text_add'] : $data['text_edit'];
