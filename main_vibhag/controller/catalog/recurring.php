...
class ControllerCatalogRecurring extends Controller {
	...
	protected function getList() {
        ...
        $this->load->autoLoadLanguage('catalog/recurring', $data);
		...
		$data['results'] = sprintf($data['text_pagination'], ($recurring_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($recurring_total - $this->config->get('config_limit_admin'))) ? $recurring_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $recurring_total, ceil($recurring_total / $this->config->get('config_limit_admin')));
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['recurring_id']) ? $data['text_add'] : $data['text_edit'];
