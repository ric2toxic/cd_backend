...
class ControllerCatalogOption extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$result = $this->model_catalog_option->editOption($this->request->get['option_id'], $this->request->post);
	...
	public function delete() {
		...
		{
			$result = $this->model_catalog_option->deleteOption($this->request->post['selected']);
	...
	protected function getList() {
        ...
        $this->load->autoLoadLanguage('catalog/option', $data);
		...
		if(!empty($this->session->data['error'])){
			$error = '';
			foreach ($this->session->data['error'] as $value) {
				$error .= $value . '<br>';
			}
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['option_id']) ? $data['text_add'] : $data['text_edit'];
		if(isset($this->request->get['option_id']) && $this->request->get['option_id'] == '5') {
			$data['form_for_option'] = 'color';
		}
		if(!empty($this->session->data['error'])){
			$error = '';
			foreach ($this->session->data['error'] as $value) {
		...
		foreach ($option_values as $option_value) {
			...
			if(!empty($data['form_for_option']) && $data['form_for_option'] == 'color') {
				$temp_option_values['hex_code'] = "";
				if(!empty($option_value['option_value_description'])) {
					foreach ($option_value['option_value_description'] as $key => $value) {
						if(!empty($value['name'])) {
							$temp_option_values['hex_code'] = $this->model_catalog_option->getColorHexCode($value['name']);
							$temp_option_values['option_value_description'][$key]['display_name'] = $value['name']." (".$temp_option_values['hex_code'].")";
						}
					}
				}
			}
			$data['option_values'][] = $temp_option_values;
		}
	...
	public function autoCompleteColor() {
		$json = array();

		if (isset($this->request->get['language_id'])) {
			
			$this->load->model('catalog/option');		
			$all_colors_by_language_id = $this->model_catalog_option->getAllColors($this->request->get['color'], $this->request->get['language_id']);
			
			if(!empty($all_colors_by_language_id)) {
				foreach ($all_colors_by_language_id as $key => $language_colors_arr) {
					$json[] = array(
											"name" => $language_colors_arr['name'],
											"hex_code" => $language_colors_arr['hex_code'],
											"display" => $language_colors_arr['name']." (".$language_colors_arr['hex_code'].")"
										);
				}
			}
		}
	...
	public function addNewColor() {
		$error = "";
		if (!empty($this->request->get['data']) && !empty($this->request->get['hexcode'])) {
			$this->load->model('catalog/option');
			
			$language_arr = json_decode(html_entity_decode(trim($this->request->get['data'])),true);
			
			$hex_code = "#".trim($this->request->get['hexcode']);
			if(empty($this->request->get['hexcode'])) {
				$error = "Please enter hex_code!";
			}
			if(!empty($language_arr)) {
				foreach ($language_arr as $language_id => $color_name) {
					if(empty($color_name)) {
						$error = "Please enter color name!";
						break;
					}
					$old_hex_code_found = $this->model_catalog_option->getColorHexCode($color_name, $language_id);
					if($old_hex_code_found) {
						$error = "Color is already added!";
					}
				}
				if(empty($error)) {
					foreach ($language_arr as $language_id => $color_name) {
						...
						$this->model_catalog_option->addNewMasterColor($color_data);
					}
				}
			}
		} else {
		...
		if(empty($error)) {
			...
			$return_data['message'] = 'Color added successfully !!!'; 
		} else {
