<?php
class ControllerLogisticBluedart extends Controller {
	...
	public function index () { 
            ...
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('Bluedart'),
                'href' => $this->url->link('logistic/bluedart&token='.$this->request->get["token"], '', 'SSL')
            );
            $data["download"] = 'sample/bluedart_sample.csv';
            $this->load->autoLoadLanguage('logistic/bluedart', $data);
            $this->load->model('logistic/bluedart');
            $this->document->setTitle($this->language->get('heading_title'));
            
            if(isset($this->request->files) && 
                empty($this->request->files["bluedart_upload"]["error"]) && 
                !empty($this->request->files)
            ){
              
                $fileName   = $this->request->files["bluedart_upload"]["name"];
               ...
               $numberOfColumnInCSV = 8;
                ...
                if($fileExten == 'csv'){
                    ...
                    $tmpName    = $this->request->files["bluedart_upload"]["tmp_name"];
                    ...
                    if(isset($sheetData) && !empty($sheetData)){
                         ...
                         if(count($columns) != $numberOfColumnInCSV){
                            ...
                            $required_columns = $this->model_logistic_bluedart->requiredCSVfileColumns();
                            echo '<pre>'; print_r($required_columns); echo '<pre>';
                            exit;
                         }

                        $this->model_logistic_bluedart->truncateBluedartPincodesTable();
                        $ind = 0;
                        $batches = array();
                        foreach ($sheetData as $key => $values){
                            for($i=0;$i<$numberOfColumnInCSV;$i++){
                                $value[$i] = $values[$i];
                            }
                        ...
                        $result_ulpload_file = $this->model_logistic_bluedart->addBluedartPincodes($batches);
                        
                        if($result_ulpload_file){
                            $data['success'] = "Succesfully Updated";
                            $this->response->setOutput($this->load->view('logistic/bluedart_upload.tpl',$data));
                        }
                        else{
            ...
            if(isset($filterPincodeNo) && !empty($filterPincodeNo)){
                $filter_pincode = $this->model_logistic_bluedart->getFilterPincodes($filter_data);
    ...
    */
    public function update_status()
    {
       ...
       if(trim($selected_pincodes))
       {
           ...
           $this->load->model('logistic/bluedart');
           $filter_pincode = $this->model_logistic_bluedart->updatePincodeStatus($pincode_data);
       }
       $url = "&filter_pincode_no=".$filter_pincode_no."&msg=Succesfully Updated";
       $this->response->redirect($this->url->link('logistic/bluedart', 
