<?php
class ControllerLogisticTruxcargo extends Controller {
	...
	public function index () { 
            ...
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('Truxcargo'),
                'href' => $this->url->link('logistic/truxcargo&token='.$this->request->get["token"], '', 'SSL')
            );
            $data["download"] = 'sample/truxcargo_sample.csv';
            $this->load->autoLoadLanguage('logistic/truxcargo', $data);
            $this->load->model('logistic/truxcargo');
            $this->document->setTitle($this->language->get('heading_title'));

            if(isset($this->request->files) && 
                empty($this->request->files["truxcargo_upload"]["error"]) && 
                !empty($this->request->files)
            ){
              
                $fileName   = $this->request->files["truxcargo_upload"]["name"];
                $fileExten  = explode(".",$fileName);
                $fileExten  = $fileExten[count($fileExten)-1];

                $numberOfColumnInCSV = 5;

                if($fileExten == 'csv'){
                   
                    $tmpName    = $this->request->files["truxcargo_upload"]["tmp_name"];
                    ...
                    if(isset($sheetData) && !empty($sheetData)){
                         ...
                         if(count($columns) != $numberOfColumnInCSV){
                            ...
                            $required_columns = $this->model_logistic_truxcargo->requiredCSVfileColumns();
                            echo '<pre>'; print_r($required_columns); echo '<pre>';
                            exit;
                         }

                        $this->model_logistic_truxcargo->truncateTruxcargoPincodesTable();
                        $ind = 0;
                        $batches = array();
                        
                        foreach ($sheetData as $key => $value){
                            
                            $value[0] = addcslashes($value[0],"'");
                            $value[1] = addcslashes($value[1],"'");
                            $value[2] = addcslashes($value[2],"'");
                            $value[3] = addcslashes($value[3],"'");
                            $value[4] = addcslashes($value[4],"'");
                        ...
                        $result_ulpload_file = $this->model_logistic_truxcargo
                                                    ->addTruxcargoPincodes($batches);
                        if($result_ulpload_file){
                            $data['success'] = "Succesfully Updated";
                            $this->response->setOutput($this->load->view('logistic/truxcargo_upload.tpl',
                                                                         $data
                                                                    ));
                        }
                        else{
            ...
            if(isset($filterPincodeNo) && !empty($filterPincodeNo)){
                $filter_pincode = $this->model_logistic_truxcargo->getFilterPincodes($filter_data);
            ...
            $this->response->setOutput($this->load->view('logistic/truxcargo_upload.tpl',$data));
    ...
    */
    public function update_status()
    {
       ...
       if(trim($selected_pincodes))
       {
           ...
           $this->load->model('logistic/truxcargo');
           $filter_pincode = $this->model_logistic_truxcargo->updatePincodeStatus($pincode_data);
       }
       $url = "&filter_pincode_no=".$filter_pincode_no."&msg=Succesfully Updated";
       $this->response->redirect($this->url->link('logistic/truxcargo', 
