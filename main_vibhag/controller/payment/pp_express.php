...
class ControllerPaymentPPExpress extends Controller {
	...
	public function index() {
	    ...
	    $this->load->autoLoadLanguage('payment/pp_express', $data);
	...
	public function refund() {
		...
		$data = array(); // Initializing the data array to be passed on to template files
	    // Autoloading the lanugage
	    $this->load->autoLoadLanguage('payment/pp_express_refund', $data);
		...
		if ($refunded != 0.00) {
			...
			$data['attention'] = $data['text_current_refunds'] . ': ' . $data['refund_available'];
		} else {
	...
	public function action() {
		if ($this->config->get('pp_express_status')) {
			...
			$data = array(); // Initializing the data array to be passed on to template files
	        // Autoloading the lanugage
	        $this->load->autoLoadLanguage('payment/pp_express_order', $data);
	...
	public function search() {
		...
		$data = array(); // Initializing the data array to be passed on to template files
	    // Autoloading the lanugage
	    $this->load->autoLoadLanguage('payment/pp_express_search', $data);
		$this->load->model('payment/pp_express');
	...
	public function viewTransaction() {
		...
		$data = array(); // Initializing the data array to be passed on to template files
	    // Autoloading the lanugage
	    $this->load->autoLoadLanguage('payment/pp_express_view', $data);
