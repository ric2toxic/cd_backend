...
require_once __DIR__ . '/system/library/sms.php';
...
list(,,$consumer_count) = $channel->queue_declare('sms_queue', false, true, false, false);
...
$callback = function($msg){
	$sms_data = unserialize(base64_decode($msg->body));
	$sms = new SMS($sms_data['message'], 
	               $sms_data['number']);
	
	$success = false;
	while ($success == false) {
		$success = $sms->sendViaQueue($sms_data['method']);
	}
