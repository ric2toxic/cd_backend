		...
		if ($order_info) {
			...
			if ($notify) {
				...
				if ($order_info['customer_id']) {
					...
					$message .= $order_info['store_url'] . 'index.php?route=account/order&order_id=' . $order_id . "\n\n";
				}
	...
	public function confirm($order_id, $order_status_id, $comment = '') {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			if (isset($order_info['email']) && !empty($order_info['email']) && $notify == 1){
				...
				$data['link'] = $order_info['store_url'] . 'index.php?route=account/order&order_id=' . $order_id;
				...
				if ($order_info['customer_id']) {
					...
					$text .= $order_info['store_url'] . 'index.php?route=account/order&order_id=' . $order_id . "\n\n";
				}
