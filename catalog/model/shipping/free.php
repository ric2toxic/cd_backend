...
class ModelShippingFree extends Model {
	function getQuote($address) {
		...
		if ($status) {
			...
			$quote_data['free'] = array(
				...
				'description'  => $this->language->get('text_extra_description'),
				'cost'         => 0.00,
				'tax_class_id' => 0,
	...
	function getAppQuote($address,$user_id) {
		$this->load->language('shipping/free');
		...
		if ($this->appcart->getSubTotal($user_id) < $this->config->get('free_total')) {
			$status = false;
		}
