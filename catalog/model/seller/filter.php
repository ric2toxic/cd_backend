<?php
class ModelSellerFilter extends Model {
	...
	public function getFilterId($filter_group,$filter_name){
		...
		$sql = "SELECT 
				...
				INNER JOIN
					...
					ON f.filter_id = fd.filter_id AND fd.name ='".$this->db->escape($filter_name)."'
				INNER JOIN 
					".DB_PREFIX. "filter_group_description fgd
					ON f.filter_group_id = fgd.filter_group_id
				WHERE 
					fgd.name = '".$this->db->escape($filter_group)."'
				" ;
	...
	public function getSellerFilter($category_ids){
		$sql = "
				SELECT 
					*, 
					(
