...
class ModelSellerCoupon extends Model {
	...
	public function getCoupons($data = array(), $store_id) {
		$sql = "SELECT coupon_id, name, code, discount, date_start, date_end, status, store_id FROM " . DB_PREFIX . "coupon WHERE store_id IN (".$store_id.")";
	...
	public function getTotalCoupons($store_id) {
		$sql =	"SELECT COUNT(*) AS total FROM " . DB_PREFIX . "coupon WHERE store_id IN (".$store_id.")";
		$query = $this->db->query($sql);
	...
	public function getCategories($data = array(), $store_id) {
		$sql = "SELECT cd.category_id, cd.name, cs.store_id FROM " . DB_PREFIX . "category_description as cd INNER JOIN oc_category_to_store as cs ON cd.category_id = cs.category_id WHERE cs.store_id IN (".$store_id.") AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getSellerProducts($data = array(), $store_id) {
		$sql = "SELECT pd.product_id, pd.name, ps.store_id, p.sku FROM " . DB_PREFIX . "product_description as pd INNER JOIN oc_product_to_store as ps ON pd.product_id = ps.product_id INNER JOIN oc_product as p ON p.product_id = p.product_id WHERE ps.store_id IN (".$store_id.")";
	...
	/*
	*	This function is to filter products from name and sku to create coupon in seller panel
	 */

	public function getSellerProductsFilteredNameSku($data = array(), $store_id) {

		$sql = "SELECT pd.product_id, pd.name, ps.store_id, p.sku FROM " . DB_PREFIX . "product_description as pd INNER JOIN oc_product_to_store as ps ON pd.product_id = ps.product_id INNER JOIN oc_product as p ON pd.product_id = p.product_id WHERE ps.store_id IN (".$store_id.") AND pd.language_id ='" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getPages($data = array(), $store_id){
		$sql = "SELECT ssp.id, ssp.title, ssp.store_id FROM " . DB_PREFIX . "seller_store_pages as ssp
				WHERE ssp.store_id IN (".$store_id.")";

		if (!empty($data['filter_name'])) {
			$sql .= " AND ssp.title LIKE '" . $this->db->escape($data['filter_name']) . "%'";
		}
