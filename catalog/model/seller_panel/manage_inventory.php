...
class ModelSellerManageInventory extends Model {
	...
	public function GetProductIsSingle($model){
		$sql ="SELECT product_id, model FROM ".DB_PREFIX."product WHERE model = '".$model.'-SNGL'."'";
	...
	public function GetProductSize($product_id){
		$sql1="SELECT ovd.name, pov.product_id, pov.product_option_value_id, pov.quantity
			...
			WHERE pov.product_id ='". $product_id."' AND ovd.language_id = 1";
	...
	public function addProduct($data) {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape(trim($data['model'])) . "', sku = '" . $this->db->escape(trim($data['sku'])) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', selling_price = '" . (float)$selling_price . "', price_per_set = '" . (float)((float)$data['price']*(int)$data['piece_in_set']) . "', piece_in_set = '" . (int)$data['piece_in_set'] . "', seller_tax = '" . (float)$data['seller_tax'] . "', commission = '" . (float)$data['commission'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', is_single = '" . (int)$data['single'] . "', date_added = NOW()");
    ...
    public function showProductInStore($product_id, $store_id, $ischeck) {
    	if($ischeck == 1){
    		...
    		$sql = "INSERT INTO " . DB_PREFIX . "product_to_store SET product_id = '" . $product_id . "', store_id = '" . $store_id . "'";
    		if($store_id == 0){
	...
	{
		$exsisting_query = $this->db->query("SELECT DISTINCT model FROM " . DB_PREFIX . "product p WHERE model LIKE " . "'" . $model . '-SNGL' . "'");
	...
	public function changeStockStatusProducts($product_id, $stock_status){
		...
		$get_model = $this->db->query("SELECT model FROM " . DB_PREFIX . "product WHERE product_id = '". $product_id ."'");
		$model = $get_model->row['model'];
		$sor_model = $model.'-SOR';
		$sor_products = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE model LIKE " . "'" . $sor_model . "'");
	...
	public function updateVacation($vacation, $seller_id){
		$sql = "UPDATE " . DB_PREFIX . "ms_seller SET vacation_mode = '". $vacation ."' WHERE seller_id = '" . (int)$seller_id . "'";
		$this->db->query($sql);
	...
	public function CreateFilterSize($set_color_size){
		...
		$sql = "INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . $filter_id . "', language_id = 1, name = '" . (int)$set_color_size . "'";
		$query = $this->db->query($sql);
	...
	public function InformSellerChangeLog($type='', $new='', $product_id=''){
		...
		$query_1 = "SELECT price, piece_in_set, model FROM ".DB_PREFIX."product WHERE product_id = '$product_id' ";
		...
		$seller_id = (int)$model_1->row['seller_id'];
	...
	public function UpdateQuantity($qn, $product_model, $product_id, $sor_product_id){
		...
		$product_sngl_ids = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE model = '" . $product_sngl_model . "'");
		...
		if(!empty($product_sngl_row_id)){
			...
			$this->db->query("UPDATE " . DB_PREFIX . "product_option_value Set quantity = '" . $qn . "' WHERE product_id = '" . (int)$product_sngl_id . "'");

			$quantity = $this->db->query("SELECT quantity FROM ".DB_PREFIX. "product_option_value WHERE product_id =" ."'". (int)$product_sngl_id."'");
			$qty = $quantity->rows;
			$temp = 0;
			foreach($qty as $qt){
				$temp = $temp + $qt['quantity'];
			}
			$sql = "UPDATE `" . DB_PREFIX . "product` SET `quantity`= '".$temp."' WHERE product_id = '".(int)$product_sngl_id."'";
			$this->db->query($sql);

			$solr_data[] = (int)$product_sngl_id;
		}
		if(!empty($sor_product_id) && isset($sor_product_id)){
			$product_sor_id = (int)$sor_product_id;
			$this->db->query("UPDATE `" . DB_PREFIX . "product` SET `quantity`= '".$qn."' WHERE product_id = '".(int)$product_sor_id."'");
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = ".$qn." WHERE product_id = '" . (int)$product_id . "'");
    ...
    public function UpdatePriceField($field, $fv, $product_id){
    	...
    	$product_id = $product_id;
    	$commission_limit = 10;
    	$record = $this->db->query("SELECT price, seller_tax, commission, piece_in_set FROM ".DB_PREFIX."product WHERE product_id = '" . (int)$product_id . "'");
        ...
        $this->InformSellerChangeLog($type, $new, $product_id);
        if (is_numeric($fv)) {
            $this->db->query("UPDATE " . DB_PREFIX . "product SET ".$field." = ".$fv.", price_per_set = ".$price_per_set.", selling_price = " . $selling_price . ", commission = " . $commission . " WHERE product_id = '" . (int)$product_id . "'"); 
        }
	}
	...
	public function UpdatePieceInSet($pis, $product_id, $sor_product_id){
		...
		$pis = $pis;
		$product_id = (int)$product_id;
		$sor_product_id = (int)$sor_product_id;
		...
		if(!empty($sor_product_id) && isset($sor_product_id)){
			...
			$this->db->query("UPDATE " . DB_PREFIX . "product SET piece_in_set = ".$pis.",price_per_set = ".$sor_price_per_set." WHERE product_id = '" . (int)$sor_product_id . "'");
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product SET piece_in_set = ".$pis.", price_per_set = ".$price_per_set." WHERE product_id = '" . (int)$product_id . "'");
	...
	public function UpdatePDField($field, $fv, $product_id, $sor_product_id){
		...
		$type = $field;
		$this->InformSellerChangeLog($type, $new, $product_id);
		$this->db->query("UPDATE " . DB_PREFIX . "product_description SET ".$field." = ".$fv." WHERE product_id = '" . (int)$product_id . "'");
		...
		if(!empty($sor_product_id) && isset($sor_product_id)){
			$sor_product_id = $sor_product_id;
			$this->db->query("UPDATE " . DB_PREFIX . "product_description SET ".$field." = ".$fv." WHERE product_id = '" . (int)$sor_product_id . "'");
			$solr_data[] = $sor_product_id;
		}
	...
	public function updateMarkupPrice($seller_id, $markup_price){
		...
		if(!empty($seller_markup->row)){
			$sql_product = "UPDATE " . DB_PREFIX . "wsb_seller_to_store SET `seller_markup_over_tp`= '" . $markup_price . "', date_modified = now() WHERE seller_id = '" . (int)$seller_id . "'";
			$this->db->query($sql_product);
		}else{
			$insert = "INSERT INTO `" . DB_PREFIX . "wsb_seller_to_store`
                    ...
                    store_id = ". SOR_STORE_ID .",
                   	seller_markup_over_tp = '" . $markup_price . "',
