...
class ModelSellerProduct extends Model {
	...
	public  function getProductFiltersData($product_id){
		...
		$q = "SELECT 
				...
				WHERE fl.filter_group_id = fgd.filter_group_id AND fgd.language_id = 1
				) as group_name
			  FROM 
		...
		if (is_array($store_id)) {
			foreach ($store_id as $store) {
				$p_id = "SELECT product_id,store_id FROM 
						...
						AND store_id = '". (int)$store ."'";
				$p_id   = $this->db->query($p_id);
				$pro_id = (int)$p_id->row['product_id'];
				$s_id   = trim($p_id->row['store_id']);
				$store = trim($store); 
				if(($pro_id == $product_id)&&($s_id == $store)){
					$sql = "UPDATE " . DB_PREFIX . "product_storeinfo SET store_id = '". $store ."', product_id = '". (int)$product_id ."', language = '". $language ."', meta_title = '". $meta_title ."', meta_keywords = '".  $meta_keyword ."', meta_description = '". $meta_description ."', modify = NOW() WHERE product_id= '". $product_id ."' AND store_id = '". $store ."'";
			...
			if(($pro_id == $product_id)&&($s_id == $store_id)){
				$sql = "UPDATE " . DB_PREFIX . "product_storeinfo SET store_id = '". (int)$store_id ."', product_id = '". (int)$product_id ."', language = '". $language ."', meta_title = '". $meta_title ."', meta_keywords = '".  $meta_keyword ."', meta_description = '". $meta_description ."', modify = NOW() WHERE product_id= '". $product_id ."' AND store_id = '". $store_id ."'";
	...
	public function get_product_store_data(){
		...
		$sql = "SELECT * from " . DB_PREFIX . "product_storeinfo WHERE product_id= '". (int)$product_id ."'";
	...
	public function get_ajax_product_store_data($store_id,$product_id){
		$sql = "SELECT * from " . DB_PREFIX . "product_storeinfo WHERE product_id= '". (int)$product_id ."' AND store_id= '".(int)$store_id."'";
