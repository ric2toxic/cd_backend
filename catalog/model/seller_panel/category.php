...
class ModelSellerCategory extends Model {
	...
	public function getcategoryfiltersdata($category_id){
		...
		(SELECT name FROM ".DB_PREFIX."filter_group_description fgd
		WHERE fl.filter_group_id = fgd.filter_group_id
		AND fgd.language_id = 1) as group_name
		FROM ".DB_PREFIX."category_filter f 
		...
		WHERE f.category_id =".(int)$category_id." GROUP BY group_name";
	...
	public function store_data(){
		...
		$language     = (int)$this->request->post['Language'] ?? 0;
		...
		if(($cate_id == $category_id)&&($s_id == $store_id)){
			$sql = "UPDATE " . DB_PREFIX . "category_storeinfo SET store_id = '". $store_id ."', category_id = '". (int)$category_id ."', language = '". $language ."', meta_title = '". $meta_title ."', meta_keywords = '".  $meta_keyword ."', meta_description = '". $meta_description ."', modify = NOW() WHERE category_id= '". (int)$category_id ."' AND store_id = '". $store_id ."'";
		}else{
			$sql = "INSERT INTO " . DB_PREFIX . "category_storeinfo SET store_id = '". $store_id ."', category_id = '". (int)$category_id ."', language = '". $language ."', meta_title = '". $meta_title ."', meta_keywords = '".  $meta_keyword ."', meta_description = '". $meta_description ."', created = NOW(), modify = NOW()";	
		}
		$query = $this->db->query($sql);
		return $query->rows;
	}
	public function get_category_store_data(){
		...
		$sql = "SELECT * from " . DB_PREFIX . "category_storeinfo WHERE category_id= '". (int)$category_id ."'";

		$query = $this->db->query($sql);
		return $query->rows;
	}
	public function get_ajax_category_store_data($store_id,$category_id){
		$sql = "SELECT * from " . DB_PREFIX . "category_storeinfo WHERE category_id= '". (int)$category_id ."' AND store_id= '".$store_id."'";
