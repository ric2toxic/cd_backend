...
class ControllerRestapiAccounts extends Controller
{
    ...
    */   
    public function getOrderLevelAccountSummary() {
      ...
      if ($this->request->server['REQUEST_METHOD'] == 'POST') {
        ...
        $filter_data = json_decode( $inputJSON, TRUE );
        ...
        if(!empty($filter_data['user_id']) ){
          ...
          $filter_data['customer_id'] = (int)$filter_data['user_id'];
          $customer_id                = (int)$filter_data['user_id'];
          ...
          if(!empty($resp)){
            ...
            $this->data_packet['statusCode'] = 200;
            $data['orders']                  = array();  
            $data['customer_summary']        = (object)array();
            $this->data_packet['data']       = $data;
