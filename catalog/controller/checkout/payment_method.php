...
class ControllerCheckoutPaymentMethod extends Controller {
	public function index() {
		...
		if (isset($this->session->data['payment_address'])) {
			...
			$results = $this->model_extension_extension->getExtensions('payment', $this->config->get('config_store_id'));
			
			$recurring = $this->cart->hasRecurringProducts();
			/*echo '<pre>'; print_r($_SESSION['URLSESSION']['citrusPayment']);
			echo '<pre>'; print_r($results); 
			echo '<pre>'; print_r($_GET); print_r($_SERVER); exit;*/
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status')) {
					$this->load->model('payment/' . $result['code']);
					if((isset($this->customer->is_dropshipper) && $this->customer->is_dropshipper == 0) || $result['code'] != 'cod'){
		...
		if($this->config->get('free_total')){
			...
			//$data['free_shipping'] = sprintf($this->language->get('free_shipping'),'10');
		}
	...
	public function save() {
		...
		if (!$json) {
			...
			$this->session->data['discount_type'] = $this->request->post['getOffer'];
