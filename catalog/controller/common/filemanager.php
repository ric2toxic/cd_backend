...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		$seller_folder_name	=	$seller['ms.nickname'];
		...
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . 'catalog/'. $seller_folder_name . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
		} else {
			$directory = DIR_IMAGE . 'catalog/'. $seller_folder_name;
		}
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => $this->url->link('common/filemanager', 'token=' . $this->session->data['token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'. $seller_folder_name))) . $url, 'SSL')
				);
	...
	public function upload() {
		...
		if (!file_exists(DIR_IMAGE . 'catalog/'. $seller_folder_name .'/')) {
            mkdir(DIR_IMAGE . 'catalog/'. $seller_folder_name, 0777, true);
        }
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . 'catalog/'. $seller_folder_name .'/'. str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
		} else {
		...
		if (!$json) {
			if (!empty($this->request->files['file']['name']) && is_file($this->request->files['file']['tmp_name'])) {
				...
				// Check for size of the images - max 1MB
