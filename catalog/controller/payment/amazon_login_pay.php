	...
	public function confirm() {
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			$product_data[] = array(
				...
				'price' => $product['price'],
				'total' => $product['total'],
				'tax' => $this->tax->getTax($product['price'], $product['tax_class_id'], '', '', $product['mrp'])
			);
		}
		...
		$order_data['ip'] = $this->request->getIpAddress;
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			$data['products'][] = array(
				...
				'total' => $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax'), $product['mrp']) * $product['quantity'])
			);
		}
