...
class ControllerPaymentcitrus extends Controller {
    ...
    public function index() {
        ...
        $data['notify_url'] = $this->url->link('payment/citrus/callback', '', 'SSL');
        $data['redir_url'] = $this->url->link('payment/citrus/callback', '', 'SSL');
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/citrus.tpl')) {
            return $this->load->view($this->config->get('config_template') . '/template/payment/citrus.tpl', $data);
        } else {
            return $this->load->view('default/template/payment/citrus.tpl', $data);
        }
    }

    public function callback() {
        //if transaction status is failed, then redirect to checkout with error
        ...
        $data['citrus_module'] = $this->config->get('citrus_module');
        $citrus_keys = $payment_gateway->getCitrusSecretKeys();
        ...
        // check for all keys, if any key matches, then break
        foreach ($citrus_keys as $citrus_key) {
          CitrusPay::setApiKey($citrus_key['secret_key'], $data['citrus_module']);
          
          if ($this->generateHmacKey($str, CitrusPay::getApiKey()) != $respSig) {
            continue; // continue to check for next citrus key
          }
          ...
          $input['order_status_id'] = $this->config->get('citrus_order_status_id');
        ...
        // no citrus key matched with response, show the error
    ...
    public function notifyInvoice() {
        ...
        $secret_key = $this->config->get('citrus_secret_key');
