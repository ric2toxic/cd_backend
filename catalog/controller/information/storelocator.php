...
class ControllerInformationStorelocator extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
            ...
            $this->document->setTitle('Store Locator');
			//$this->document->setTitle($information_info['meta_title']);
			$this->document->setDescription('Store Locator');
			$this->document->setKeywords('Store Locator');
          ...
          if(CONFIG_IS_MOBILE == 1)
           {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/storelocator.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/information/storelocator.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/information/storelocator.tpl', $data));
			}
