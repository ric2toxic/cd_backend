...
class ControllerSellerCoupon extends Controller {
	...
	protected function getList() {
		...
		$store_id = implode(',' , $seller_store_id);
		...
		$data['breadcrumbs'] = $this->MsLoader->MsHelper->setBreadcrumbs(array(
			...
			array(
				'text' => $this->language->get('ms_account_coupon'),
				'href' => $this->url->link('seller/coupon', '', 'SSL'),
			)
		));
		...
		$coupon_total = $this->model_seller_coupon->getTotalCoupons($store_id);

		$results = $this->model_seller_coupon->getCoupons($filter_data, $store_id);
		 
		foreach ($results as $result) {
			$data['coupons'][] = array(
				'coupon_id'  => $result['coupon_id'],
				'name'       => $result['name'],
				'store'		 =>	$this->model_seller_coupon->getstorename($result['store_id']),
				'code'       => $result['code'],
		...
		$data['sort_store_id'] = $this->url->link('seller/coupon', 'sort=store_id' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('seller/coupon', 'page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('default/template/multiseller/coupon_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		foreach ($products as $product_id) {
			$product_info = $this->model_seller_coupon->getProduct($product_id);
		...
		foreach ($categories as $category_id) {
			$category_info = $this->model_seller_coupon->getCategory($category_id);
		...
		$this->response->setOutput($this->load->view('default/template/multiseller/coupon_form.tpl', $data));
	}
	...
	public function history() {
		...
		$this->response->setOutput($this->load->view('default/template/multiseller/coupon_history.tpl', $data));
	}
...
//			}
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_seller_coupon->getCategories($filter_data, $store_id);
	...
	public function autocompleteProduct() {
		...
		if (isset($this->request->get['filter_all'])) {
			...
			$filter_data = array(
				...
				'filter_sku' => $this->request->get['filter_all'],
				'sort'        => 'name',
				'order'       => 'ASC',
				'start'       => 0,
				'limit'       => 5
			...
			$results = $this->model_seller_coupon->getSellerProductsFilteredNameSku($filter_data, $store_id);
	...
	public function autoComplatePages(){
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_seller_coupon->getPages($filter_data, $store_id);
