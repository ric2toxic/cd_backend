...
class ControllerSellerCatalogSeller extends ControllerSellerCatalog {
	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			$this->data['sellers'][] = array(
					...
					'description' => utf8_substr(strip_tags(html_entity_decode($result['ms.seller_description'], ENT_QUOTES, 'UTF-8')), 0, 200) . '..',
	...
	public function profile() {
		...
		$this->data['seller']['description'] = html_entity_decode($seller['ms.seller_description'], ENT_QUOTES, 'UTF-8');
	...
	public function products() {
		...
		if (!empty($products)) {
			...
			//	} else {
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$price = $this->currency->format($this->tax->calculate($product_data['price'], $product_data['tax_class_id'], $this->config->get('config_tax'), $product_data['mrp']));
				} else {
					$price = false;
				}

				if ((float)$product_data['special']) {
					$special = $this->currency->format($this->tax->calculate($product_data['special'], $product_data['tax_class_id'], $this->config->get('config_tax'), $product_data['mrp']));
				} else {
					$special = false;
				}
				...
				$this->data['seller']['products'][] = array(
						'product_id' => $product['product_id'],
						'set_description' => utf8_substr(strip_tags(html_entity_decode($product['pd.set_description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('config_product_description_length')) . '..',
						'minimum'     => $product['p.minimum'] > 0 ? $product['p.minimum'] : 1,
						'text_in_stock'=>sprintf($this->language->get('text_in_stock'), $product['p.quantity']),
						'text_sold_out'=>sprintf($this->language->get('text_sold_out'), 0),
						'text_out_of_stock'=>$this->language->get('text_out_of_stock'),
						'quantity'=>(int)$product['p.quantity'],
						'stock_status'=>$product['p.stock_status'],
						'thumb' => $image,
