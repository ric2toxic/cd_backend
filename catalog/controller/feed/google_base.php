...
class ControllerFeedGoogleBase extends Controller {
	public function index() {
		if ($this->config->get('google_base_status')) {
			...
			$results_data = $this->model_catalog_product->getProducts();
			...
			foreach ($products as $product) {
				if ($product['description']) {
					...
					if ((float)$product['special']) {
						$output .= '<g:price>' .  $this->currency->format($this->tax->calculate($product['special'], $product['tax_class_id'], '', $product['mrp']), $currency_code, $currency_value, false) . '</g:price>';
					} else {
						$output .= '<g:price>' . $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], '', $product['mrp']), $currency_code, $currency_value, false) . '</g:price>';
					}
