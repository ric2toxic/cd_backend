...
class ControllerApiShipping extends Controller {
	public function address() {
		...
		if ($this->cart->hasShipping()) {
			if (!isset($this->session->data['api_id'])) {
				...
				/**
				 * Commented by KD to make last name not mandatory
				 **/
	...
	public function methods() {
		...
		} elseif (isset($this->request->post['json_data'])) {

			if (!$json) {
				// Shipping Methods
				$json['shipping_methods'] = array();
                $order_information = json_decode(base64_decode($this->request->post['json_data']));
                $product_array =  (array) $order_information->order_products;
                $shipping_array =  (array) $order_information->shipping;
                //echo "<pre>"; print_r($product_array); die;
                $weight = $this->backendcart->getWeight($product_array);
				...
				foreach ($results as $result) {
					if ($this->config->get($result['code'] . '_status')) {
						...
						$quote = $this->{'model_shipping_' . $result['code']}->getQuote($shipping_array, $weight);
