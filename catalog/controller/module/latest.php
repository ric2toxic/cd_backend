...
class ControllerModuleLatest extends Controller {
	public function index($setting) {
		...
		if($this->config->get('config_store_id') == INTERNATIONAL_STORE_ID || $this->config->get('config_store_id') == SOR_STORE_ID ){
		...
		if ($results) {
            ...
            if(isset($this->request->get['amp']) && $this->request->get['amp'] == 1){
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/latest-amp.tpl')) {
					echo $this->load->view($this->config->get('config_template') . '/template/module/latest-amp.tpl', $data);
				} else {
