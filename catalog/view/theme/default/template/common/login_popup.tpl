               ...
               <span class="flag_area"><i class="country-flag flagstrap-icon flagstrap-<?php echo strtolower($country_iso_code); ?>" style="margin-right: 10px;"></i></span>
                ...
                <div class="mobile_details_panel" id="save_reg_telephone">
                  <div class="flagstrap flag_box" id="select_country" data-input-id="country_code" data-input-name="country_code" data-selected-country="IN"></div>
                    ...
                    <?php echo $send_otp_form['reg_telephone']; ?>
                ...
                success: function(data)
                {
                    ...
                    else
                    {
                      ...
                      else
                      {
                        ...
                        {
                          $('#button-cart, .button-cart-list').trigger("click");
                        }
                        else
                        {
...
$( document ).ready(function() {
 ...
 var postcode_max = 6;
    ...
    $.validator.addMethod("GSTNumberValid", function (textObj, element)
     {
         var reggstin = /^([0-9]){2}([A-Z]){5}([0-9]){4}([A-Z]){1}([A-Z0-9]){1}([A-Z]){1}([0-9]){1}?$/;
      ...
      $("#register_form").validate({
          rules: {
             ...
             password: {
              ...
              postcode: {
                  required: true,
                  minlength: postcode_min,
                  maxlength: postcode_max
              },
              address_1: {
                  required: true,
                  minlength: 3,
       ...
       if($(this).val().length == 6)
       {
         ajax = $.ajax({
                ...
                success: function(data)
                {
                  ...
                  if(!data['error'])
                  {
                    ...
                    {
                     $("#country_id, #zone_id, #city").prop("disabled", false);
                     $("#zone_id").html('<option value="0"><?php echo $entry_zone; ?></option>');
                     $("#country_id option[value='0']").prop("selected",true);
                     $("#city").val('');
                    ...
                    else
                    {
                      ...
                      $("#city").val(data['city']);
                      $("#zone_id").html('<option value="'+data['zone_id']+'">'+data['state']+'</option>');
        ...
        else if(INTERNATIONAL_STORE == 1 && $(this).val().length > 3 && $(this).val().length < 6)
        {
          $("#register_form input[type=submit]").val('continue').prop("disabled", false);
          $("#country_id, #zone_id, #city").prop("disabled", false);
          $("#zone_id").html('<option value="0"><?php echo $entry_zone; ?></option>');
       ...
       $("#country_id").change(function(e) {
        ...
        ajax = $.ajax({
                ...
                success: function(data)
                {
                  var result = '<option value="0">'+data['zone_title']+'</option>';
     ...
     $(document).delegate('#login_link_header, #footer-login, #dropshipper_login_link, #button-cart, #wishlist-total, .addtocart_bottem, .addtocart_new, .button-cart-list', 'click', function(e)
     {    
