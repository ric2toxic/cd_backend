...
<div class="clearfix"></div>
  <div class="row"><?php echo $column_left; ?>
    ...
    <div class="alert alert-success"><?php echo $success; ?></i></div>
    <?php } ?>
    <div class="alert alert-error"></i></div>
    
      <h1 class="account_title"><?php echo $heading_title; ?></h1>
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" class="form-horizontal" id="bank_form">
        <fieldset>
          <legend>Your Bank Details</legend>
          <div class="form-group required">
            <label class="col-sm-3 control-label"><?php echo $entry_account_holder_name; ?> </label>
            <div class="col-sm-8">
              <input type="text" name="bank_ac_holder_name" value="<?php echo $bank_ac_holder_name; ?>" placeholder="<?php echo $entry_account_holder_name; ?>" id="bank_ac_holder_name" class="form-control" />
          <?php if (isset($error["error_account_holder_name"])) { ?>
          ...
          <div class="form-group required">
            ...
            <div class="col-sm-8">
              <input type="text" name="bank_ac_number" value="<?php echo $bank_ac_number; ?>" placeholder="<?php echo $entry_account_no; ?>" id="bank_ac_number" class="form-control" />
              <?php if (isset($error["error_account_number"])) { ?>
              <div class="text-danger"><?php echo $error["error_account_number"]; ?></div>
              <?php } ?>
            </div>
          </div>
          <div class="form-group required">
            <label class="col-sm-3 control-label"><?php echo $entry_ifsc_code; ?></label>
            <div class="col-sm-8">
              <input type="text" name="ifsc_code" value="<?php echo $ifsc_code; ?>" placeholder="<?php echo $entry_ifsc_code; ?>" id="ifsc_code" class="form-control sample_ifsc_code" />
              <?php if (isset($error["error_ifsc_code"])) { ?>
              <div class="text-danger"><?php echo $error["error_ifsc_code"]; ?></div>
              <?php } ?>
            </div>
         </div>
         <legend>Your UPI Details</legend>
          <div class="form-group">
            <label class="col-sm-3 control-label"><?php echo $entry_customer_vpa; ?> </label>
            <div class="col-sm-8">
              <input type="text" name="customer_vpa" value="<?php echo $customer_vpa; ?>" placeholder="<?php echo $entry_customer_vpa; ?>" id="customer_vpa" class="form-control" />
          <?php if (isset($error["error_customer_vpa"])) { ?>
              <div class="text-danger"><?php echo $error["error_customer_vpa"]; ?></div>
              <?php } ?>
            </div>
          </div>

           <div class="form-group" id="otp_area" <?php if (!isset($error["error_otp"])) { ?> style="display: none;" <?php } ?>>
            <label class="col-sm-3 control-label"> OTP </label>
            <div class="col-sm-8">
              <input type="text" name="otp" placeholder="OTP" id="otp" class="form-control" />
              <a id="otp_load" href="javascript:;" onclick="resend_otp();">Did'nt get OTP?</a>
              <?php if (isset($error["error_otp"])) { ?>
              <div class="text-danger"><?php echo $error["error_otp"]; ?></div>
              <?php } else { ?>
              <div class="text-danger">Please enter 4 digit otp which has been sent on your mobile number or email.</div>
              <?php } ?>
            </div>
          </div>
         <div class="ifsc_details bank_details_block col-md-8 col-md-offset-3">
         </div>
         <div class="buttons clearfix">
            <div class="col-sm-3"></div>
            <div class="col-sm-8 form_buttons_width">
               <div class="pull-left" style="margin-left: 16px;"><a href="<?php echo $back; ?>" class="btn btn-default">Back</a></div>
               <div class="pull-right" style="margin-right: 20px;">
                 <input type="submit" value="Submit" class="btn btn-continue" />
               </div>
           </div>
        </div>
      </form>
...
<script type="text/javascript">
$(document).ready(function(){
    var ifsc_code_obj = $("input[name=ifsc_code]").val();
    getIfscCodeDetails("input[name=ifsc_code]",ifsc_code_obj);
});
...
function getIfscCodeDetails(obj, ifsc_code){
      ...
      if(ifsc_code.length == 11){
          $.ajax({
              ...
              // dataType : 'json',
              beforeSend: function(){
                  //$('.ms-account-profile').html('<div class="loadSpinner">Checking...</div>').show();
                  $('.error-ms-bank').remove();
                  $(obj).parent().find('.sample_ifsc_code').after('<span style="border:0" class="error-ms-bank"><i class="fa fa-refresh fa-spin fa-3x fa-fw"></i> <span class="error_text">checking...</span></span>');
              },
              success:function(json){
                  if(typeof(json) == 'object'){
                      ...
                      //html += '&nbsp;&nbsp;<div class="form-group required col-sm-12"><input type="checkbox" class="confirmMyBank" checked  name="seller[bank_confirm_bank]" value="yes" />&nbsp;&nbsp;<label class="control-label">Confirm Bank Details</label></div>';

                      $('.bank_details_block').html(html);

                  }else{
                      //$('#ms-bank-submit-button').attr('disabled',true);
                      $('.error-ms-bank').remove();
                      $(obj).parent().find('.sample_ifsc_code').after('<span style="border:0" class="error_text error-ms-bank">'+json+'</span>');
                      $('.bank_details_block').html('');
                      $('#ms-bank-submit-button').find('span').text('Save');
...
<script type="text/javascript">
    ...
    $("#bank_form").submit(function(event) {
      $("#otp_load").html('<i class="fa fa-circle-o-notch"></i>');
       var otp = $("#otp").val();
       if(otp == '')
       {
