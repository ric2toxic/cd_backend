...
<div class="border_top"></div>
 <?php if ($products) {
  ...
  <?php $loop = 0;
          foreach ($products as $product) { $productInfo[$product['product_id']] = $product['row_data'];  ?>

              <div class="c-product-card new_ outofstock installments_ mastercard c-product-card_js_inited c-product-card_view_grid c-product-shortlist__item" data-product-id="<?php echo $product['product_id']; ?>">
          
                   <div data-product-id="<?php echo $product['product_id']; ?>" class="wishlist_btn_right addtowishlist remove_btn">
                   <a class="remove_prd_wishlist" data-product-id="<?php echo $product['product_id']; ?>" data-href="<?php echo $product['remove']; ?>" data-toggle="tooltip" title="Remove">
                      <button  title="" data-toggle="tooltip" id="wishlist_<?php echo $product['product_id']; ?>" class="addtowishlist" type="button" data-original-title="<?php echo $button_remove; ?>"><i id="wishlist_heart_<?php echo $product['product_id']; ?>" class=" fa fa-heart custom_heart"></i>
                      </button></a>
                  </div> 
                  
                  <div class="c-product-card__img-placeholder">
                      <a href="<?php echo $product['href']; ?>" class="c-product-card__img-placeholder-inner" target="_blank">
                          <span class="c-img-lazy  c-product-card__img  c-img-lazy_js_inited c-img-lazy_loaded">
                              <img src="<?php echo $product['thumb']; ?>" class="largeImage c-img-lazy__img">
                          </span>
                      </a>
                  </div>
                  <div class="c-product-card__price-block">
                      <div class="c-quick-buy  c-product-card__buy-button c-quick-buy_js_inited">
                        ...
                        <?php if(isset($product['row_data']['stock_status']) && $product['row_data']['stock_status'] == 'out of stock') { ?>
                          <button id="want_designe2" type="button" class="btn btn-checkout comment_btn" html-data="<?php echo $product['product_id']; ?>-<?php echo $product['minimum']; ?>" data-product-id="<?php echo $product['product_id']; ?>"><i class="fa fa-shopping-cart"></i> I want this design</button>
                        <?php } ?>  


                           <a class="fancybox ellipsis productInfoAjax"  href="<?php echo $product['href']; ?>" data-product-id="<?php echo $product['product_id']; ?>" target="_blank">
                         ...
                         <?php if(isset($product['row_data']['stock_status']) && $product['row_data']['stock_status'] == 'out of stock') { ?>
                           <button class="c-quick-buy__button c-button c-button_size_big out_of_stock">Out Of Stock</button>
                         <?php } else { ?>
                          
                          <?php if(count($product['options']) > 0) { ?>
                          <a href="<?php echo $product['href'] ?>"  data-product-id="<?php echo $product['product_id'] ?>"  className="ellipsis" target="_blank">
                           <button class="c-quick-buy__button c-button c-button_size_big c-button_color_old-orange  c-button_js_inited">ADD TO CART</button>
                          </a> 
                           <?php } else { ?>
                            <button class="c-quick-buy__button c-button c-button_size_big c-button_color_old-orange  c-button_js_inited"  onclick="cart_add('<?php echo $product['product_id']; ?>', '<?php echo $product['minimum']; ?>');">ADD TO CART</button>
                           <?php } ?>
                  ...
                  <div class="c-product-card__description">
                    ...
                    <div class="c-product-card__price">
                      <span class="c-product-card__price-final"><?php echo $product['price']; ?></span> 
                    </div>
                    ...
                    <?php if($product['rating'] == 5) {  ?>
                      <span class="label label-success">Excellent Quality</span>
                    <?php } ?> 

                    <?php if($product['rating'] == 4) {  ?>
                      <span class="label label-warning">Good Quality</span>
                    <?php } ?>

                    <?php if($product['rating'] <= 3) {  ?>
                     <span class="label label-danger">Average Quality</span>
                    <?php } ?>

                    <?php if($product['margin_percentage'] != '') {  ?>
                     <span class="c-product-margin"> Your margin <?php echo $product['margin_percentage']; ?>%</span>
                    <?php } ?> 
     
                    </div>
                    <div class="product_card_description pd_description_hight">
...
<div class="modal fade add_new_address" id="comment_popup_<?php echo $product['product_id']; ?>" role="dailog" data-backdrop="static" data-keyboard="false">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div id="want_header_popup" class="modal-header want_header">
                                        <button type="button" data-dismiss="modal" class="close">&times;</button>
                                        <h4 class="modal-title">I want this design.</h4>
                                    </div>

                                    <div class="modal-body popup-q-body"> 

                                        <div class="popup-title preorder_title_box msg_body_<?php echo $product['product_id'];?>">
                                        ...
                                        <div class="popup-footer">
                                             <button type="button" class="btn deliver_btn preeorder_submit_bnt pull-right" data-product-id="<?php echo $product['product_id'];?>"><?php echo $comment_popup_send;?></button>
                                            <div class="clearfix"></div>
                                        </div> 
                                    </div>
 ...
 <script>
   ...
   $('.comment_btn').click(function(){
     //alert($(this).attr('data-product-id'));
       ...
       $('.comment_popup').hide();
       $('#comment_popup_' + $(this).attr('data-product-id')).modal("show");
   });


    $('.preeorder_submit_bnt').click(function(){
      var data = this;
...
$('.alert-danger').hide();
  ...
  if(popup_comment == '')
  {
    $('.msg_body_'+product_id).before('<div class="alert alert-danger"><i class="fa fa-check-circle"></i>Please enter your message</div>');
   }
   else
   {
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          $('.msg_body_'+product_id).before('<div class="alert alert-success"><i class="fa fa-check-circle"></i>'+data['message']+'</div>');
        }
      });
   }   
    });
...
$(document).ready(function(){
    ...
    //==========Remove wishlist product local storage===========//
      $('.remove_prd_wishlist').click(function(e){
        e.preventDefault();
        var all_store_product = [];
        var selectProductId=$(this).data('product-id');

        web_engage_product_detail(parseInt(selectProductId, 10), 0, 'we-custom-removefromwishlist');
          ...
          var previousProduct=JSON.parse(localStorage.getItem("recentWishList"));
                  ...
                  var dateTime = (dt.getFullYear() +"/"+ (dt.getMonth()+1) +"/"+ dt.getDate()+" "+dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds());
                       $.each(previousProduct, function(index, value) {
                           ...
                           if (value.product_id != selectProductId) {
