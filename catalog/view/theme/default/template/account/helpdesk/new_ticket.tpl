...
<div class="container-fluid width_fix">
    <div class="clearfix"></div>
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" class="form-horizontal" id="new_ticket"> 
        <fieldset> 
          <legend><?php echo $create_ticket; ?></legend>  

          <div class="form-group">                                                       
            <label class="col-sm-2 control-label" for="input-subject"><?php echo "Ticket Type:-"; ?></label>
            <div class="col-sm-9">            

              <select class="form-control not_null" name="ticket_type_id" required>
              <option value = "0" selected="selected" >Please select a issue type</option>
                <?php foreach($ticket_type as $select){        
                                ?>
                      <option value='<?php  print_r($select['group_id']);?>'><?php print_r($select['group_name']); ?> </option>               
                <?php                                            
                  }?>
          ...
          <div class="form-group required">
            ...
            <label class="col-sm-2 control-label" for="input-subject"><?php echo $subject; ?></label>
             <div class="col-sm-9">                    
                <input type="text" name="subject" placeholder="<?php echo $entry_subject; ?>" id="input-subject" class="form-control required_fields "  required/>
                <span class="error" style="color:red; display: none; "></span>
             </div>
          </div>

          <div class="form-group required">
              <label class="col-sm-2 control-label" for="input-description"><?php echo $description; ?></label>
           
               <div class="col-sm-9">
               
                    <textarea class="form-control input-description required_fields" name="description" rows="4"  placeholder="<?php echo $entry_description; ?> " required ></textarea>
                </div> 
           </div>
            
            <div class="form-group ">       
            
              <label class="col-sm-2 control-label" for="attachment"><?php echo $attachment; ?></label>
              
              <div class="col-sm-9">          
                
                 <input id="attachment" name ="attachment_file" type="file" class="file">
        ...
        <div class="col-sm-9 form_buttons_width">
          ...
          <div class="pull-right">                   
            <input type="submit" id="ticket_form_submit" value="<?php echo $button_continue; ?>" class="btn btn-continue" />
          </div>
        </div>
        </div>
      </form>
...
$("#ticket_form_submit").click(function(e) {
e.preventDefault();
var success=1;
  
   var selectoption = $('.not_null').val(); 
   if(selectoption == 0){
       $('.not_null').addClass('error');
       $('.not_null').css("border-color","red");
       $('.error').show();  
       success = 0;
  ...
  $('#new_ticket').find('.required_fields').each(function(){    
    
    if($(this).val().match(/([\<])([^\>]{1,})*([\>])/i) !== null){
...
$(".required_fields").keyup(function(e) {
e.preventDefault();
