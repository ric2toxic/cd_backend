          ...
          <div id="content" class="<?php echo $class; ?> box_shadow_none">
            ...
            <div class="pull-right"><a href="<?php echo $create; ?>" class="btn btn-continue"><?php echo $button_new_address; ?></a></div>
              ...
              <?php echo $all_tickets; ?>
                ...
                <div class="dropdown hide_dropdown">
                  <button class="btn btn-continue dropdown-toggle" type="button" data-toggle="dropdown">Ticket Status
                  <span class="caret"></span></button>
                  <ul class="dropdown-menu">
                    <li><a onclick="showTickets();"  id = "all_tickets">All tickets</a></li>
                    <li><a onclick="showTickets('OPEN');">Open </a></li>                  
                    <li><a onclick="showTickets('CLOSED');">Closed </a></li>                  
                  </ul>
                </div>   
              ...
              ?>
                <a href="<?php echo $ticket['url']; ?>">
                  <div class="helpdesk_div row">
                    <!-- <div class="helpdesk_box"> -->
                      <div class="helpdesk_letter">
                        <?php
                          $capital = ucwords($ticket['ticket_creater']);
                          $first_letter = substr($capital, 0,1);
                          echo $first_letter;
                        ?>
                      </div>
                                
                      <div class="col-sm-6  word_break ">           
                        <b style="color:black;font-size:15px;">
                            <?php echo $ticket['subject'].'#'.$ticket['ticket_id']; ?>
                        </b> 
                        <p> 
                          <?php  echo 'Assigned to :- '.$ticket['assigned_to_agent']; ?>  <br>
                          <?php echo $ticket['created_at']; ?>                          
                        </p>
                      </div>
                      <div class="col-sm-4" style="color:white; background-color:#eba600; border-radius:16px; width:200px; text-align: center; margin-top:16;  margin-right: 5">
                         <?php echo $ticket['status']?> 
                      </div>
                      <?php if(!empty($ticket['unread_reply'])){ ?> 
                                     <span class="conversation_read_count">
                                        <?php echo $ticket['unread_reply']; ?>
                                     </span>
                      <?php } ?> 
                    <!-- </div> -->
                  </div>
              ...
              <?php
               ...
               echo "<b><center>No tickets</center></b>";
              }              
              ?> 
              <center>
               <?php 
                ...
                $startpage = 1;
                if(isset($pagination) && isset($data_count) ){

                  $endpage = ceil($data_count/$pagination['perpage']);

                  $curpage      = $pagination['curpage'];
                  $perpage      = $pagination['perpage'];
                  $nextpage     = $pagination['curpage'] + 1;
                  $previouspage = $pagination['curpage'] - 1;
                } 
               ...
               if (isset($endpage) &&  $endpage > 1) { ?>

                        <ul class="pagination">

                            <?php if($curpage != $startpage){ ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?php echo $page_url; ?>&page=<?php echo $startpage ?>&status=<?php echo $status;  ?>" tabindex="-1" aria-label="Previous">
                                        <span aria-hidden="true">&laquo;</span>
                                        <span class="sr-only">First</span>
                                    </a>
                                </li>                        <li class="page-item">
                                    <a class="page-link" href="<?php echo $page_url; ?>&page=<?php echo ($curpage-1)?>&status=<?php echo $status;  ?>" tabindex="-1" aria-label="Previous">
                            ...
                            <?php if ($curpage >= 2) { ?>
                                <li class="page-item"><a class="page-link" href="<?php echo $page_url; ?>&page=<?php echo $previouspage;  ?>&status=<?php echo $status;  ?>"><?php echo $previouspage ?></a></li>
                            <?php } ?>


                            <li class="page-item active"><a class="page-link" href="<?php echo $page_url; ?>&page=<?php echo $curpage?>"><?php echo $curpage ?></a></li>                <?php if ($curpage != $endpage) { ?>

                                <li class="page-item"><a class="page-link" href="<?php echo $page_url; ?>&page=<?php echo $nextpage; ?>&status=<?php echo $status; ?>"><?php echo $nextpage ?></a></li>
                            <?php } ?>

                            <?php if($curpage != $endpage){ ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?php echo $page_url; ?>&page=<?php echo ($curpage+1) ?>&status=<?php echo $status; ?>" aria-label="Next">
                            ...
                            <?php if($curpage != $endpage){ ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?php echo $page_url; ?>&page=<?php echo $endpage ?>&status=<?php echo $status;  ?>" aria-label="Next">
