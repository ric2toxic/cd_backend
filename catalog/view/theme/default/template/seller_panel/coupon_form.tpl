        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-coupon" class="form-horizontal">
          ...
          <div class="tab-content">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <select name="store_id" id="input-store_id" class="form-control">
                      ...
                      <?php  if($value['store_id'] == $store_id){ ?>selected <?php } ?>
                      >
                        <a><label class="control-label"><?php echo $value['name']; ?></label></a>
                      </option>
                      <?php } ?>
                  </select>
                </div>
            ...
            <?php }else{ ?>
              <div class="form-group">
                <div class="col-sm-10">
                  <input type="hidden" name="store_id" value="<?php echo $storeid[0]['store_id']; ?>" class="form-control" />
                </div>
              </div>
            <?php } ?>
          </div>
...
$('input[name=\'product\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=seller/coupon/autocompleteProduct&filter_all=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
                  var label_name = item['name']+'('+item['sku']+')';
					return {
						label: label_name,
...
$('input[name=\'category\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=seller/coupon/autocompleteCategory&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
$('#history').load('index.php?route=seller/coupon/history&coupon_id=<?php echo $coupon_id; ?>');
//--></script>
  <?php } ?>
