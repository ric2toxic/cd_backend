<form class="form-horizontal">
    ...
    <?php if ($addresses) { ?>
    <div class="radio col-md-8 use_save_address">
        <div class="payment_address existing" data-type="existing" >
            <?php echo $text_address_existing; ?></div>
    </div>
    <div class="radio col-md-4 new_button_address_link new_address">
      ...
      ?>
            <?php
                if(count($addresses)== 1){
                    $addClass = "selected_address";
                    $address_value = $address['address_id'];
                    $hidden_city_value = $address['city'];
                    $hidden_address1_value = $address['address_1'];
                }else{
                    $addClass = "";
                    $address_value ="";
                    $hidden_city_value = "";
                    $hidden_address1_value = "";
                }
            ?>
            <div class="col-md-4 payment_address<?php echo $address['address_id']; ?>" >
                <div class="address_block billing_address <?php echo $addClass; ?><?php //echo $cssclass; ?>" data-address-id="<?php echo $address['address_id']; ?>" hidden-city-value="<?php echo $address['city']; ?>" hidden-address1-value="<?php echo $address['address_1']; ?>">
            ...
            <input type="hidden" name="hidden_address_1" value="<?php echo $address['address_1']; ?>">
            <input type="hidden" name="hidden_city" value="<?php echo $address['city']; ?>">
            <?php } ?>
    ...
    <div class="buttons">
        ...
        <div class="pull-right">
            <div class="continue continue_collapse payment_address_continue" data-address-id="<?php echo $address_value;?>" hidden-city-value="<?php echo $hidden_city_value; ?>" hidden-address1-value="<?php echo $hidden_address1_value;?>"><span> <?php echo $button_continue; ?></span></div>
        </div>
    </div>
    <?php } ?>
    <br />
...
<script type="text/javascript"><!--
    ...
    $('div.payment_address').on('click', function() {
        if ($(this).attr('data-type') == 'new') {
            ...
            $('div.new_address').removeClass('new_button_address_link col-md-4');
            $('div.use_save_address').removeClass('col-md-8');
            ...
            $('div.new_address').addClass('new_button_address_link col-md-4');
            $('div.use_save_address').addClass('col-md-8');
    ...
    $('div.billing_address').on('click', function() {
        ...
        $('div.continue_collapse').attr('hidden-city-value',$(this).attr('hidden-city-value'));
        $('div.continue_collapse').attr('hidden-address1-value',$(this).attr('hidden-address1-value'));
    ...
    $('div.continue_collapse').on('click', function() {
        //if(($('input[name=\'hidden_address_1\']').val() != '') && ($('input[name=\'hidden_city\']').val() != '')){
            if($(this).attr('data-address-id')!=''){
                if($(this).attr('hidden-city-value')!='' && $(this).attr('hidden-address1-value')!=''){
                    $('input[name=\'address_id\']').val($(this).attr('data-address-id'));
                    $('input#button-payment-address').trigger('click');
                }else{
                    alert('Please select your another address!');
                }
            }else{
                alert('Please select your address!');
            }
        //}else{
            //alert('Please fill your first address and your city!');
        //}
...
<script type="text/javascript"><!--
    ...
    /*
    // $('#collapse-payment-address select[name=\'country_id\']').on('change', function() {
    country_id = $('#collapse-payment-address input[name=\'country_id\']').attr('value');
    $.ajax({
        url: 'index.php?route=checkout/checkout/country&country_id=' + country_id,
        dataType: 'json',
        beforeSend: function() {
