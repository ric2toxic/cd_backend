...
$(document).ready(function() {
	...
	*/
    ...
    $("div#search input.form-control").keyup(function(){
        ...
        var text = $(this).val();
        if(text.length >= 2){
            var url = "http://localhost/wholesalebox/index.php?route=product/product/getsearchproduct";
	...
	'add': function(product_id) {
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['success']) {
					$('#content').parent().before('<div class="alert alert-success addinwishlist"><i class="fa fa-check-circle"></i> ' + json['success'] + '<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
				}

				if (json['info']) {
					$('#content').parent().before('<div class="alert alert-info addinwishlist"><i class="fa fa-info-circle"></i> ' + json['info'] + '<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
				}
...
$(document).ready(function() {
	$('.post_code').keyup(function () {
		...
		if (char_count > 5) {
			...
			var pincode = $(this).val(); //$('.' + parent_class + ' .post_code').val();
			console.log(ctoken + "-------"+pincode);
			$.ajax({
