<?php
  ...
  * Time: 5:17 PM
  */
  require_once(DIR_SYSTEM . '/library/operations/orders/orderpartialshipped.php');
  require_once( 'orderstatehelper.php');
  Class OrderPartialShippedTest extends OpenCartTest{
    private $helper;
    private $order_state_object;
    public function __construct(){
      $this->helper = new OrderStateHelper($this);
      $this->order_state_object = $this->helper->createOrderStateObject('OrderPartialShipped');
    }
    ...
    /**
     * [testDoActionIfPostStateNotExists
     *  This method tested the exceptions comes or not if calling change order state action without post_state ]
     *
     */
    public function testDoActionIfPostStateNotExists(){
      $this->order_state_object = $this->helper->createOrderStateObject('OrderFullShipped');
