 ...
 * Time: 5:17 PM
 */
 require_once(DIR_SYSTEM . 'library/operations/orders/orderplacing.php');
 require_once('orderstatehelper.php');
 Class OrderPlacingTest extends OpenCartTest{
   private $helper;
   ...
   public function testChangeOrderIfPaymentIsNotSet(){
     $this->order_state_object = $this->helper->createOrderStateObject('OrderPlacing');
     $action = 'changeOrderState';
     $data = array('post_state'=> 'PLCD');
     $this->helper->doActions($this->order_state_object,$action,$data);
     $this->helper->deleteOrder($this->helper->order_id);
     $this->assertContains('checkPaymentStaus',$this->order_state_object->getPostAction());
   }

   public function testChangeOrderIfPayentIsDone(){
     $this->order_state_object = $this->helper->createOrderStateObject('OrderPlacing');
     $action = 'changeOrderState';
     $data = array('post_state'=> 'PLCD','payment_transaction' => array('status'=>false));
     $this->helper->deleteOrder($this->helper->order_id);
     $this->helper->doActions($this->order_state_object,$action,$data);
   }

   public function testChangeOrderIfPayentIsPending(){
     $this->order_state_object = $this->helper->createOrderStateObject('OrderPlacing');
     $action = 'changeOrderState';
     $data = array('post_state'=> 'PLCD','payment_transaction' => array('status'=>false));
     $this->helper->doActions($this->order_state_object,$action,$data);
     $this->helper->deleteOrder($this->helper->order_id);
     $this->assertContains('genPaymentLink',$this->order_state_object->getPostAction());
   }
   /**
